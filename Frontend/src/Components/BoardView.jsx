...
const BoardView = () => {
    ...
    const fetchBoardData = async () => {
        ...
        try {
            ...
            const boardRes = await axios.get(`${API_URL}/boards/${boardId}`, { headers });
            setBoard(boardRes.data);

            const listsRes = await axios.get(`${API_URL}/lists/${boardId}`, { headers });
            ...
            setError(err.response?.data?.message || "Failed to load board data. Check membership.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleListCreate = (newList) => {
        setLists(prevLists => [...prevLists, newList].sort((a, b) => a.position - b.position));
    };
    ...
    useEffect(() => {
        ...
        const results = allCards.filter((card) => {
            const matchesTitle = card.title.toLowerCase().includes(lowerCaseSearchTerm);
            ...
            const matchesLabel = Array.isArray(card.labels)
                ? card.labels.some((label) => label.toLowerCase().includes(lowerCaseSearchTerm))
                : false;
    ...
    if (error || !board) {
        return (
            <div className="flex-grow flex items-center justify-center min-h-screen bg-white">
                <p className="text-red-600">Error: {error || "Board not found or access denied."}</p>
            </div>
        );
    }
